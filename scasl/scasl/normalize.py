...
def knn_impute(df_fillna, df_prob, k=5):
    ...
    d = np.zeros((m, m))
    dist_bar = trange(m - 1)
    dist_bar.set_description('Calculating distances')
    for i in dist_bar:
        for j in range(i, m):
    ...
    df_knn = df_prob.copy()
    imputation_bar = trange(m)
    imputation_bar.set_description('executing knn imputation')
    for i in imputation_bar:
        na_map = df_prob.iloc[:, i].isna().astype(int)
...
def normalize(df_path, num_iter, k):
    ...
    mat = np.zeros((m, n, 2))
    mat[:, :, 0] = df_fillna.values.T
    mat[:, :, 1] = indicator.values.T
